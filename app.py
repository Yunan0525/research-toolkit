...
# ── Path bootstrap ────────────────────────────────────────────────────────────
# Ensures the project root is on sys.path so that `components`, `utils`, etc.
# can always be imported regardless of which directory Streamlit was launched
# from. Safe to run multiple times; inserting a duplicate path has no effect.
